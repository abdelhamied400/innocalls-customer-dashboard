...
type ZendeskActionsCellProps = {
  credential: ZendeskCredential;
};

const ZendeskActionsCell = ({ credential }: ZendeskActionsCellProps) => {
  ...
  const handleToggle = async () => {
    try {
      ...
      if (credential.isDeleted) {
        await zendeskCredentialService.restore(credential.id);
      } else {
        await zendeskCredentialService.delete(credential.id);
      }

      queryClient.setQueryData<ZendeskCredential[]>(
        ["zendesk-credentials"],
        (oldData) => {
...
export default ZendeskActionsCell;
