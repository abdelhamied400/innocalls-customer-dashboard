...
type ZohoActionsCellProps = {
  credential: ZohoCredential;
};

const ZohoActionsCell = ({ credential }: ZohoActionsCellProps) => {
  ...
  const handleToggle = async () => {
    try {
      ...
      if (credential.isDeleted) {
        await zohoCredentialService.restore(credential.id);
      } else {
        await zohoCredentialService.delete(credential.id);
      }

      queryClient.setQueryData<ZohoCredential[]>(
        ["zoho-credentials"],
        (oldData) => {
...
export default ZohoActionsCell;
