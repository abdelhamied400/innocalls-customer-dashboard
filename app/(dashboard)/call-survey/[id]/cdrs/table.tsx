...
import SurveyCdrsHead from "./head";

const SurveyCdrsTable = () => {
  ...
  const { data, isLoading, isError, error } = useLocalizedQuery({
    queryKey: ["call-survey-cdrs", id, filters, pagination],
    queryFn: async () =>
      await callSurveyService.fetchCdrs(id, {
        ...filters,
  ...
  return (
    <div className="rounded-lg flex-1 flex flex-col overflow-hidden">
      ...
      >
        <SurveyCdrsHead filters={filters} setFilters={handleFiltersChange} />
...
export default SurveyCdrsTable;
