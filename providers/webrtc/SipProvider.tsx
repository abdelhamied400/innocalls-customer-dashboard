...
export const SipProvider = ({ children }: SipProviderProps) => {
  ...
  const [extension, setExtension] = useState<ExtensionWithCredentials | null>(globalState.extension);
  const [extensionState, setExtensionState] = useState<ExtensionState>(globalState.extensionState);
  ...
  const login = useCallback(async (extension: ExtensionWithCredentials) => {
    const now = Date.now();
    ...
    try {
      ...
      const userAgent = await sipManager.ensureSingleSession(extension.uri, extension.password);
      ...
      toast({
        ...
        description: `Unable to connect to SIP server: ${error instanceof Error ? error.message : 'Unknown error'}`,
        variant: "destructive"
      });
    } finally {
  ...
  const spy = useCallback((extension: string) => {
    if (!ua) {
      toast({
        title: "Error",
