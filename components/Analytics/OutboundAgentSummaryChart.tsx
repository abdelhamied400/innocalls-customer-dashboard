...
const OutboundAgentSummaryChart: React.FC<OutboundAgentSummaryChartProps> = ({ data }) => {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-purple-600 text-sm">{`Answer Rate: ${Math.round((agent.answeredCalls / agent.totalCalls) * 100)}%`}</p>
          <p className="text-orange-600 text-sm">{`Avg Duration: ${Math.floor(agent.avgDuration / 60)}:${(agent.avgDuration % 60).toString().padStart(2, '0')}`}</p>
        </div>
      );
    }
    return null;
