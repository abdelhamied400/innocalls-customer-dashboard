...
const ExitTimeoutChart: React.FC<ExitTimeoutChartProps> = ({ data }) => {
  ...
  const queueStats = [
    { name: "Avg Wait Time", value: `${Math.floor(data.avgWaitTimeBeforeTimeout / 60)}:${(data.avgWaitTimeBeforeTimeout % 60).toString().padStart(2, '0')}` },
    { name: "Min Wait Time", value: `${Math.floor(data.minWaitTimeBeforeTimeout / 60)}:${(data.minWaitTimeBeforeTimeout % 60).toString().padStart(2, '0')}` },
    { name: "Max Wait Time", value: `${Math.floor(data.maxWaitTimeBeforeTimeout / 60)}:${(data.maxWaitTimeBeforeTimeout % 60).toString().padStart(2, '0')}` },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-purple-50 p-4 rounded-lg border">
          <h3 className="text-lg font-semibold text-purple-800">Total Timeouts</h3>
          <p className="text-2xl font-bold text-purple-600">{data.totalTimeoutCalls}</p>
        </div>
        <div className="bg-indigo-50 p-4 rounded-lg border">
          <h3 className="text-lg font-semibold text-indigo-800">Unique Callers</h3>
          <p className="text-2xl font-bold text-indigo-600">{data.uniqueCallersTimeout}</p>
        </div>
        <div className="bg-cyan-50 p-4 rounded-lg border">
          <h3 className="text-lg font-semibold text-cyan-800">Queues Affected</h3>
          <p className="text-2xl font-bold text-cyan-600">{data.queuesWithTimeouts}</p>
        </div>
      </div>
          ...
          <div className="space-y-3">
            {queueStats.map((stat, index) => (
              <div key={index} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                ...
                <span className="text-purple-600 font-semibold">{stat.value}</span>
              </div>
            ))}
          </div>
        </div>
